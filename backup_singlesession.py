...
# HR-facing API (analyse, schedule, dashboard reads, keys) — matches the frontend contract
import api_routes
app.include_router(api_routes.router)

# allow the React frontend (different origin) to call us
...
# ─── SCHEDULER WORKER (robust, DB-backed, survives restarts) ──
SCHEDULER_POLL_SEC = 30   # how often to check for due interviews

def scheduler_worker():
    """
    Background loop: every poll, ask Supabase for interviews due now and launch them.
    Source of truth is the DB, so a server restart loses nothing — on reboot we resume.
    NOTE: the interview ENGINE is still single-session (global state), so we launch one
    at a time and skip launching while an interview is already active.
    """
    print(f"🗓️  scheduler worker started (poll every {SCHEDULER_POLL_SEC}s)")
    while True:
        try:
            due = db.due_scheduled_interviews()
            for row in due:
                # don't start a new interview while one is active (single-session engine)
                if state["interview_started"] and not state["interview_over"]:
                    print("🗓️  interview active — deferring due rows to next poll")
                    break
                print(f"🗓️  launching scheduled interview {row['id']}")
                result = deploy_bot(row["meeting_url"])
...
@app.get("/trigger-bot")
def trigger_bot(meeting_url: str):
    """Immediate deploy (manual/testing). Scheduled joins go through the scheduler worker."""
    return deploy_bot(meeting_url)

def deploy_bot(meeting_url: str) -> dict:
    """Reset state, create the Recall bot for meeting_url, start join+intro. Returns status dict."""
    if state["interview_started"] and not state["interview_over"]:
        print("⚠️ deploy_bot refused: an interview is already active (single-session engine)")
        return {"status": "busy", "detail": "an interview is already in progress"}
    ...
    if r.status_code == 201:
        ...
        threading.Thread(target=wait_for_join_and_speak, args=(bot_id, intro), daemon=True).start()
...
async def schedule_interview(request: Request):
    """
    Called by the HR dashboard (or the operator UI below).
    Body: {meeting_url, candidate_email, candidate_name, role, delay_minutes}
    - emails the invite now (so the candidate has the link),
    - stores the schedule in Supabase,
    - the scheduler worker auto-deploys the bot at scheduled_for.
    """
    ...
    row = db.create_scheduled_interview(
        meeting_url=meeting_url, scheduled_for_iso=scheduled_for.isoformat(),
        candidate_email=email or None, candidate_name=name or None, role=role or None)

    email_ok = False
    if email:
        email_ok = scheduler.send_invite(email, name, meeting_url, role=role, when=when_human)
        if row and email_ok:
            db.update_scheduled_interview(row["id"], email_sent=True)

    return {"status": "scheduled",
            "scheduled_for": scheduled_for.isoformat(),
            "join_in_minutes": delay,
            "email_sent": email_ok,
...
def console():
    """Minimal operator page to drive scheduling for demos/testing.
    The real HR dashboard (other team) calls POST /schedule-interview directly."""
    return """<!doctype html><html><head><meta charset="utf-8">
<title>Interview Scheduler — Operator Console</title>
<style>
 ...
 button{margin-top:16px;padding:10px 16px;border:0;border-radius:6px;background:#2563eb;color:#fff;font-size:15px;cursor:pointer}
 #out{margin-top:16px;padding:12px;border-radius:6px;background:#f3f4f6;font-size:13px;white-space:pre-wrap}
 small{color:#666}
</style></head><body>
<h1>Schedule an Interview</h1>
<small>HR creates the Teams/Meet meeting manually (admit everyone), then schedules here.</small>
<label>Meeting URL *</label><input id="url" placeholder="https://teams.microsoft.com/...">
<label>Candidate email</label><input id="email" placeholder="candidate@example.com">
<label>Candidate name</label><input id="name" placeholder="Jane Doe">
<label>Role</label><input id="role" placeholder="Senior Engineer">
<label>Bot joins in</label>
<select id="delay"><option value="10">10 minutes</option><option value="30">30 minutes</option></select>
<button onclick="go()">Schedule + send invite</button>
<div id="out"></div>
<script>
async function go(){
  const out=document.getElementById('out'); out.textContent='Scheduling...';
  const body={meeting_url:url.value,candidate_email:email.value,candidate_name:name.value,
              role:role.value,delay_minutes:parseInt(delay.value)};
  try{
    const r=await fetch('/schedule-interview',{method:'POST',headers:{'Content-Type':'application/json'},
